                ...
                {!!channels && channels.length > 0 &&
                    channels.map(channel =><>
                        <div className="channellist" onContextMenu={(e) => rightonclick(e, channel.id)} >
                            <NavLink className="friendslistlist" to={`/channels/${channel.serverId}/${channel.id}`} ><div><i class="fa-solid fa-hashtag fa-lg"/></div> {channel.name.length> 7 ? channel.name.slice(0, 7)+"...":channel.name}{channelId ==channel.id}
                            </NavLink>
